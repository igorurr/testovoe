...
const InputFeedback = ({ error }) =>
  error ? <Styles.ErrorText className="error">{error}</Styles.ErrorText> : null;
  ...
  return (
    <Styles.InputCoworkerField className={className}>
      <label htmlFor={id} error={error}>
        <Styles.InputCoworkerLabelP>{label}</Styles.InputCoworkerLabelP>
      </label>
      <input
        id={id}
    ...
    return (
      <form onSubmit={handleSubmit}>
        <CoworkerField
          ...
          placeholder="Моё имя"
          error={
            (this.state.wasSubmittingOnce || touched.firstName) &&
            errors.firstName
          }
        ...
        <CoworkerField
          ...
          error={
            (this.state.wasSubmittingOnce || touched.lastName) &&
            errors.lastName
          }
        ...
        <CoworkerField
          ...
          label="Дата рождения"
          error={
            (this.state.wasSubmittingOnce || touched.dateWasBorn) &&
            errors.dateWasBorn
          }
        ...
        <CoworkerField
          ...
          error={
            (this.state.wasSubmittingOnce || touched.email) && errors.email
          }
          value={values.email}
          onChange={handleChange}
          onBlur={handleBlur}
...
const MyEnhancedForm = withFormik({
  validationSchema: Yup.object().shape({
    ...
    lastName: Yup.string()
      ...
      .matches(
        /^([А-Яа-я]*)$/u,
        "Фамилия должна быть написана на русском языке"
      )
      .required("Поле пусто"),
    email: Yup.string()
      .email("Невалидный email адрес")
      .required("Поле пусто"),
    post: Yup.string().required("Поле пусто"),
